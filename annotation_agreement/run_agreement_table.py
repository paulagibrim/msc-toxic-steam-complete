...
Usage:
    ...
    python run_agreement_table.py --assume-missing-agrees ...
"""
import argparse
...
def parse_args():
    ...
    parser.add_argument(
        "--assume-missing-agrees", action="store_true",
        help=(
            "Treat a blank annotation as agreeing with the annotators who did label that review, "
            "instead of dropping the review. Required to reproduce the published table (four of P's "
            "en labels are missing from the spreadsheet), but it is reconstruction, not data, and it "
            "can only ever raise agreement - leave it off for a defensible figure."
        ),
    )
...
def main():
    ...
    if args.assume_missing_agrees:
        info("--assume-missing-agrees is ON: blank annotations are filled, agreement is biased upward")
    ...
    for lang in languages:
        ...
        halves = [at.agreement_table(sheet, args.assume_missing_agrees) for sheet in sheets]
        entry = {"language": lang}
        for half in halves:
    ...
    save_summary(
        {
            ...
            "missing_annotations": (
                "assumed to agree with the other annotators"
                if args.assume_missing_agrees
