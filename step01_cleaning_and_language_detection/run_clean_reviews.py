...
def main():
    ...
    # Default is 30s - too tight when many CPU-bound worker processes are
    # competing for cores (e.g. during the langdetect map_partitions pass):
    # a worker can be too busy to service another worker's data request in
    # time, which then reads as a dead connection ("Connection timed out"/
    # "Connection reset") even though the worker is still alive, just slow
    # to respond - raising the timeout gives it more slack before Dask
    # decides the connection has failed.
    dask.config.set({
